...
BitArray.getIntersection = function (bitArray1, bitArray2) {
  ...
  for (var i = 0; i < len; i++) {
    result.setAt(
      i,
      BitArray._intersect(bitArray1.getAt(i), bitArray2.getAt(i))
    );
  }
  return result;
};
...
BitArray.getDifference = function (bitArray1, bitArray2) {
  ...
  for (var i = 0; i < len; i++) {
    result.setAt(
      i,
      BitArray._difference(bitArray1.getAt(i), bitArray2.getAt(i))
    );
  }
  return result;
};
...
BitArray._intersect = function (bit1, bit2) {
  return bit1 === BitArray._ON && bit2 === BitArray._ON
    ? BitArray._ON
    : BitArray._OFF;
};
